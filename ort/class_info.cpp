...
bool ort_is_same_or_has_base(std::uint32_t derived, std::uint32_t base)
{
    if (derived == base)
    {
        return true;
    ...
    else
    {
        ...
        const info::class_info* cinfo = index->get_class_info(info::class_id(derived));
        if (cinfo)
        {
            int n = int(cinfo->base_class_ids().size());
            for (int i = 0; i < n; ++i)
            {
                info::class_id id = cinfo->base_class_ids()[i];
                if (ort_is_same_or_has_base(to_underlying(id), base))
                {
                    return true;
                }
            }
