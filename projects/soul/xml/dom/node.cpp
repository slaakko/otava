...
ParentNode* Node::Parent() const noexcept
{
    return parent;
}

void Node::SetParent(ParentNode* parent_) noexcept
{ 
    parent = parent_; 
}
...
void Node::SetOwnerDocument(Document* ownerDocumnent_) noexcept 
{ 
    ownerDocument = ownerDocumnent_; 
}
...
void Node::WalkAncestor(NodeOperation& operation)
{
    ...
    if (p)
    {
        p->WalkAncestorOrSelf(operation);
    }
}
