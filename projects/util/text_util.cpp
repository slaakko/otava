...
std::string ToUpperNarrow(const std::string& s)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        result.append(1, char(std::toupper(s[i])));
    }
    return result;
}
...
std::string ToLowerNarrow(const std::string& s)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        result.append(1, char(std::tolower(s[i])));
    }
    return result;
}
...
{
    std::string s;
    s.append(1, HexNibble(std::uint8_t(x >> 4))).append(1, HexNibble(std::uint8_t(x & 0x0F)));
    return s;
}
