...
Project::Project(const std::string& filePath_, const std::string& name_) :
    fileMap(nullptr), filePath(filePath_), name(name_), initialized(false), scanned(false), loaded(false), target(Target::program), projectId()
{
    rootDir = util::Path::GetDirectoryName(filePath);
}
...
{
    moduleMapper.AddRoot(rootDir);
    for (const auto& referenceFilePath : referenceFilePaths)
    {
        std::string referenceRoot = util::GetFullPath(util::Path::Combine(rootDir, util::Path::GetDirectoryName(referenceFilePath)));
...
{
    std::string rp = util::GetFullPath(util::Path::Combine(rootDir, referenceFilePath));
    ...
    otava::symbols::Context* context)
{
    ...
    for (const auto& m : modules)
    {
        ...
        std::cout << ">" << m->Name() << "\n";
#endif
        m->SetModuleFilePath(rootDir, config, optLevel, context, configurations);
        otava::symbols::Cardinality exportedModuleNameCount = m->ExportedModuleNameCount();
        for (otava::symbols::Index i = otava::symbols::Index(0); i < otava::symbols::Index(otava::symbols::ToUnderlying(exportedModuleNameCount)); ++i)
        {
        ...
        for (otava::symbols::Index i = otava::symbols::Index(0); i < otava::symbols::Index(otava::symbols::ToUnderlying(importedModuleNameCount)); ++i)
        {
...
void Project::MapFiles()
{
    ...
    {
        std::string interfaceFilePath = util::GetFullPath(util::Path::Combine(rootDir, interfaceFileName));
    ...
    {
        std::string sourceFilePath = util::GetFullPath(util::Path::Combine(rootDir, sourceFileName));
...
bool Project::UpToDate(const std::string& config, int optLevel, const std::set<std::string>& configurations) const
{
    ...
    for (const auto& m : modules)
    {
        if (m)
        {
            ...
            {
                std::string moduleFilePath = otava::symbols::MakeModuleFilePath(rootDir, config, optLevel, m->Name(), configurations);
...
otava::symbols::SymbolId Project::GetIrId(const std::string& groupFullName, otava::symbols::Cardinality arity)
{
    auto it = irIdMap.find(std::make_pair(groupFullName, arity));
    if (it != irIdMap.end())
    {
        return it->second;
    }
    return otava::symbols::zeroSymbolId;
}

void Project::SetIrId(const std::string& groupFullName, otava::symbols::Cardinality arity, otava::symbols::SymbolId irId)
{
    irIdMap[std::make_pair(groupFullName, arity)] = irId;
}
