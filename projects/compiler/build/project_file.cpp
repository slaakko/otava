    ...
    const std::set<std::string>& configurations)
{
    ...
    if (!referencedProjects.empty())
    {
        for (const auto& referencedProject : referencedProjects)
        {
            ...
            {
                std::string releaseReference = util::Path::Combine(util::Path::Combine(util::Path::Combine(util::Path::Combine(
                    referencedProject->RootDir(), "bin"), config),
            ...
            {
                std::string debugReference = util::Path::Combine(util::Path::Combine(util::Path::Combine(referencedProject->RootDir(), "bin"), config),
