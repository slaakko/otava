...
RegisterGroup::RegisterGroup() : kind(RegisterGroupKind::max), nonvolatile(false), regs()
{
    regs.resize(16);
}

RegisterGroup::RegisterGroup(RegisterGroupKind kind_) : kind(kind_), nonvolatile(false), regs()
{
    regs.resize(16);
}
    ...
    registers(registers_), localRegisterCount(0), localXMMRegisterCount(0), usedGlobalRegs(GlobalReg::none), isChildFnRegisterPool(false)
{
    ...
    localRegisterCount = int(localRegisterPool.size());
    localXMMRegisterCount = int(localXMMRegisterPool.size());
