...
void FunctionGroupSymbol::Read(Reader& reader)
{
    ...
    for (Index i = Index(0); i < ToIndex(fnCount); ++i)
    {
    ...
    for (Index i = Index(0); i < ToIndex(defCount); ++i)
    {
    ...
    const std::vector<TypeSymbol*>& specialization, TemplateDeclarationSymbol* templateDeclaration, bool hasSpecialization, Context* context) 
{
    ...
    for (FunctionSymbol* function : fns)
    {
        ...
        if (templateDeclaration)
        {
            ...
            for (Index i = Index(0); i < ToIndex(templateDeclaration->Arity()); ++i)
            {
        ...
        if (function->Arity() == Cardinality(parameterTypes.size()))
        {
            ...
            if (found)
            {
                ...
                {
                    for (Index i = Index(0); i < ToIndex(function->Arity()); ++i)
                    {
