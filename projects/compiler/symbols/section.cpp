...
Section::Section(Module* module_, SectionKind kind_) : module(module_), kind(kind_), entriesRead(false), readOnly(false), headerRead(false)
{
}
...
Symbol* Section::GetSymbol(SymbolId symbolId, Context* context)
{
    ...
    if (readOnly)
    {
        ...
        if (fit != entryMap.end())
        {
            ...
            std::unique_ptr<Symbol> s(symbol);
            sectionSymbols.push_back(std::move(s));
            return symbol;
        }
        ...
        {
            Module* importedModule = context->GetModuleMapper()->GetModule(moduleId);
            if (!importedModule)
            {
                importedModule = GetModule()->GetImportedModuleById(moduleId, context);
            }
            if (importedModule)
            {
                Symbol* symbol = importedModule->GetSymbolTable()->GetSymbol(symbolId, context);
                if (symbol)
                {
                    MapSymbol(symbol, context);
                    return symbol;
                ...
                else
                {
                    context->SetException(MakeException("symbol id " + std::to_string(ToUnderlying(symbolId)) + " not found from module id " +
                        std::to_string(ToUnderlying(importedModule->Id())) + " (" + importedModule->Name() + ")"));
                }
            }
            else
            {
