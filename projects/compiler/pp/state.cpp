...
State::State(OtavaLexer* lexer_) : lxr(lexer_)
{
    currentSection.reset(new Section(nullptr));
#ifdef OTAVA
    Define("OTAVA", 1);
    Define("OTAVA_FOR_OTAVA", 1);
#else
    Define("OTAVA", 1);
#endif
}
...
void State::Include()
{
    if (!currentSection->Skip())
    {
        ...
        std::cout << "warning: include directive '" << directiveStr << "' skipped" << "\n";
    }
}
...
State* LexerStateMap::GetState(OtavaLexer* lxr)
{
