...
LiteralOperatorIdNode::LiteralOperatorIdNode(const soul::ast::Span& span_, int fileIndex_, Node* id_) noexcept :
    UnaryNode(NodeKind::literalOperatorIdNode, span_, fileIndex_, id_)
{
}

Node* LiteralOperatorIdNode::Clone() const
{
    LiteralOperatorIdNode* clone = new LiteralOperatorIdNode(GetSpan(), FileIndex(), Child()->Clone());
    clone->SetId(Id());
    return clone;
}
