...
BeginAccessGroupNode::BeginAccessGroupNode(const soul::ast::Span& span_, int fileIndex_, Node* accessSpecifier_) noexcept :
    UnaryNode(NodeKind::beginAccessGroupNode, span_, fileIndex_, accessSpecifier_)
{
}

Node* BeginAccessGroupNode::Clone() const
{
    BeginAccessGroupNode* clone = new BeginAccessGroupNode(GetSpan(), FileIndex(), Child()->Clone());
    clone->SetId(Id());
    return clone;
}
...
{
    PureSpecifierNode* clone = new PureSpecifierNode(GetSpan(), FileIndex());
    clone->SetId(Id());
    return clone;
}
