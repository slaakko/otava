...
LambdaIntroducerNode::LambdaIntroducerNode(const soul::ast::Span& span_, int fileIndex_, Node* capture_) noexcept :
    CompoundNode(NodeKind::lambdaIntroducerNode, span_, fileIndex_), capture(capture_)
{
}
...
Node* LambdaIntroducerNode::Clone() const
{
    ...
    LambdaIntroducerNode* clone = new LambdaIntroducerNode(GetSpan(), FileIndex(), clonedCapture);
    clone->SetId(Id());
    return clone;
}
...
{
    CurrentObjectCopyCapture* clone = new CurrentObjectCopyCapture(GetSpan(), FileIndex());
    clone->SetId(Id());
    return clone;
}
...
{
    CurrentObjectByRefCapture* clone = new CurrentObjectByRefCapture(GetSpan(), FileIndex());
    clone->SetId(Id());
    return clone;
}
