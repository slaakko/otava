...
{
    EnumClassNode* clone = new EnumClassNode(GetSpan(), FileIndex());
    clone->SetId(Id());
    return clone;
}
...
{
    EnumStructNode* clone = new EnumStructNode(GetSpan(), FileIndex());
    clone->SetId(Id());
    return clone;
}
...
EnumeratorDefinitionNode::EnumeratorDefinitionNode(const soul::ast::Span& span_, int fileIndex_, Node* enumerator_, Node* value_) noexcept :
    CompoundNode(NodeKind::enumeratorDefinitionNode, span_, fileIndex_), enumerator(enumerator_), value(value_)
{
}
...
Node* EnumeratorDefinitionNode::Clone() const 
{
    ...
    EnumeratorDefinitionNode* clone = new EnumeratorDefinitionNode(GetSpan(), FileIndex(), enumerator->Clone(), clonedValue);
    clone->SetId(Id());
    return clone;
}
