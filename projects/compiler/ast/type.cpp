...
DeclTypeSpecifierNode::DeclTypeSpecifierNode(const soul::ast::Span& span_, int fileIndex_, Node* expr_) noexcept :
    CompoundNode(NodeKind::declTypeSpecifierNode, span_, fileIndex_), expr(expr_)
{
}

Node* DeclTypeSpecifierNode::Clone() const
{
    DeclTypeSpecifierNode* clone = new DeclTypeSpecifierNode(GetSpan(), FileIndex(), expr->Clone());
    clone->SetId(Id());
    return clone;
}
...
PlaceholderTypeSpecifierNode::PlaceholderTypeSpecifierNode(const soul::ast::Span& span_, int fileIndex_) noexcept :
    CompoundNode(NodeKind::placeholderTypeSpecifierNode, span_, fileIndex_), declType(false)
{
}

PlaceholderTypeSpecifierNode::PlaceholderTypeSpecifierNode(const soul::ast::Span& span_, int fileIndex_, Node* typeConstraint_, bool declType_) noexcept :
    CompoundNode(NodeKind::placeholderTypeSpecifierNode, span_, fileIndex_), typeConstraint(typeConstraint_), declType(declType_)
{
}
...
Node* PlaceholderTypeSpecifierNode::Clone() const
{
    ...
    PlaceholderTypeSpecifierNode* clone = new PlaceholderTypeSpecifierNode(GetSpan(), FileIndex(), clonedTypeConstraint, declType);
    clone->SetId(Id());
    return clone;
}
