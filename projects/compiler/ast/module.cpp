...
ExportDeclarationNode::ExportDeclarationNode(const soul::ast::Span& span_, int fileIndex_, Node* exprt_, Node* subject_) noexcept :
    CompoundNode(NodeKind::exportDeclarationNode, span_, fileIndex_), exprt(exprt_), subject(subject_)
{
}
...
Node* ExportDeclarationNode::Clone() const
{
    ...
    ExportDeclarationNode* clone = new ExportDeclarationNode(GetSpan(), FileIndex(), exprt->Clone(), clonedSubject);
    clone->SetId(Id());
    return clone;
}
