...
struct ModuleHeader
{
    ...
    FileOffset importedModuleTableOffset;
    Length importedModuleTableLength;
};

struct ImportedModuleTableEntry
{
    inline ImportedModuleTableEntry() noexcept : moduleId(zeroModuleId), moduleNameOffset(notFoundOffset) {}
    inline ImportedModuleTableEntry(ModuleId moduleId_, StringOffset moduleNameOffset_) noexcept : moduleId(moduleId_), moduleNameOffset(moduleNameOffset_) {}
    void Write(Writer& writer);
    void Read(Reader& reader);
    ModuleId moduleId;
    StringOffset moduleNameOffset;
};
...
public:
    ...
    inline FileOffset GetImportedModuleTableOffset() const noexcept { return header.importedModuleTableOffset;  }
    inline Length GetImportedModuleTableLength() const noexcept { return header.importedModuleTableLength; }
    ...
    Module* GetImportedModuleById(ModuleId moduleId, Context* context);
    void AddImportedModule(Module* module);
    void SetModuleFilePath(const std::string& root, const std::string& config, int optLevel, Context* context, const std::set<std::string>& configurations);
    ...
    void WriteImportedModuleTable(Writer& writer);
    void ReadImportedModuleTable();
    void ReadImportedModuleTable(Reader& reader);
...
private:
    ...
    std::vector<ImportedModuleTableEntry> importedModuleTable;
    bool importedModuleTableRead;
    void Read();
...
public:
    ...
    void PrintModules(std::ostream& s);
...
private:
    ...
    std::map<std::string, Module*> moduleNameMap;
    std::map<ModuleId, Module*> moduleIdMap;
    SymbolIndexMap symbolIndexMap;
